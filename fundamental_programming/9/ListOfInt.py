...
def MaxList(Li):
    if IsOneElmt(Li):
        ...
        return Max2(LastElmt(Li), MaxList(Head(Li)))
...
def ListPlus(Li1, Li2):
    if Dimensi(Li1) == 0:
        ...
        return Konso(FirstElmt(Li1) + FirstElmt(Li2), ListPlus(Tail(Li1), Tail(Li2)))
    ...
    if IsListInt(Li):
        if IsEmpty(Li):
            ...
            if x <= FirstElmt(Li):
                ...
                return Konso(FirstElmt(Li), Insert(x, Tail(Li)))
    else:
        return 0
...
def Insort(Li):
    if IsListInt(Li):
        if IsEmpty(Li):
            ...
            return Insert(FirstElmt(Li), Insort(Tail(Li)))
    else:
        return 0
...
def NbOcc(X, Li):
    if IsListInt(Li):
        if IsOneElmt(Li):
            ...
            if X == FirstElmt(Li):
                return 1 + NbOcc(Tail(Li))
            else:
                return NbOcc(Tail(Li))
    else:
        return 0
